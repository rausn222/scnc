...
export default function SCIDetail({ row }: Props) {
  ...
  const detailScenarioId =
    acceptedId === "no-action" ||
    acceptedId === "iut" ||
    acceptedId === "moq" ||
    acceptedId === "iut-moq" ||
    acceptedId === "iut-moq-break"
      ? acceptedId
      : null;
...
function ConfidenceScoreInline({ score }: { score: number }) {
...
function iutMaterialKey(laneKey: string, code: string) {
  return `${laneKey}__${code}`;
}

function defaultIutMaterialPossible(): Record<string, boolean> {
  const state: Record<string, boolean> = {};
  for (const lane of IUT_TRANSFER_LANES) {
    const laneKey = `${lane.from}→${lane.to}`;
    for (const mat of IUT_LANE_REQUIREMENTS[laneKey] ?? []) {
      state[iutMaterialKey(laneKey, mat.code)] = true;
    }
  }
  return state;
}
...
function IutLaneRow({
  ...
  materialPossible: Record<string, boolean>;
  onToggleMaterial: (laneKey: string, code: string) => void;
}) {
  const laneKey = `${lane.from}→${lane.to}`;
  ...
  return (
    <div
      className="px-4 py-2.5 flex flex-wrap items-center gap-x-4 gap-y-2"
      ...
      {materials.length > 0 ? (
        <div className="flex flex-wrap items-center gap-2 flex-1 min-w-0">
          {materials.map((mat) => {
            ...
            const matKey = iutMaterialKey(laneKey, mat.code);
            const possible = materialPossible[matKey] ?? true;
            return (
              <div
                key={mat.code}
                className="inline-flex flex-wrap items-center gap-2 px-2 py-1.5 rounded-lg text-xs"
              ...
              >
                ...
                <label className="flex items-center gap-1.5 cursor-pointer shrink-0">
                  ...
                  <button
                    ...
                    aria-label={`${mat.type} ${mat.code} transfer possible`}
                    onClick={() => onToggleMaterial(laneKey, mat.code)}
                    className="relative w-9 h-4 rounded-full transition-colors shrink-0"
                    style={{ backgroundColor: possible ? C.green : "#cbd5e1" }}
                  >
                    <span
                      className="absolute top-0.5 w-3 h-3 rounded-full bg-white transition-transform"
                      style={{ left: possible ? 18 : 2 }}
                    />
                  </button>
                </label>
              </div>
...
function MoqBreakMaterialRow({
  ...
  return (
    <div
      className="px-4 py-2 flex flex-wrap items-center gap-x-2 gap-y-1.5"
      style={{ borderTop: "1px solid #f8fafc" }}
    >
      <span
      ...
      <span className="text-xs font-semibold shrink-0" style={{ color: C.navy }}>
        {mat.code}
      </span>
      <span
        className="text-xs truncate max-w-[180px] shrink-0"
        style={{ color: "#64748b" }}
        title={mat.description}
      >
        {mat.description}
      ...
      {suppliers.length > 0 ? (
        <>
          ...
          {suppliers.map((supplier) => (
            <span
              key={supplier.name}
              className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md shrink-0"
              style={{ backgroundColor: "#f8fafc", border: "1px solid #e2e8f0" }}
            >
              <span className="text-[10px] font-medium" style={{ color: C.navy }}>
                {supplier.name}
              </span>
              <ConfidenceScoreInline score={supplier.confidenceScore} />
            </span>
          ))}
        </>
      ) : null}
...
function SimulationAssumptionsStep({ newCbuRow }: { newCbuRow: CBURow | null }) {
  ...
  const [iutMaterialPossible, setIutMaterialPossible] = useState(defaultIutMaterialPossible);
        ...
        >
          <div className="px-4 py-3 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs mt-0.5" style={{ color: "#64748b" }}>
                Mark transfer feasibility for each RM and PM on the lane
              </p>
            </div>
          </div>
          <div style={{ borderTop: "1px solid #f1f5f9" }}>
            {IUT_TRANSFER_LANES.map((lane) => {
              ...
              return (
                <IutLaneRow
                  key={laneKey}
                  lane={lane}
                  materialPossible={iutMaterialPossible}
                  onToggleMaterial={(lk, code) =>
                    setIutMaterialPossible((prev) => {
                      const key = iutMaterialKey(lk, code);
...
function ScenarioComparisonStep({
  ...
  return (
    <StepSection
      ...
      subtitle="Click a card to view component breakdown"
      headerAction={dateModeToggle}
    >
      <div
...
function ScenarioDetailView({
  ...
  row: CBURow;
  scenarioId: "no-action" | "iut" | "moq" | "iut-moq" | "iut-moq-break";
}) {
  const scenarioLabel =
    scenarioId === "iut"
      ? "IUT scenario"
      : scenarioId === "moq"
        ...
        : scenarioId === "iut-moq"
          ? "IUT + MOQ scenario"
          : scenarioId === "iut-moq-break"
            ? "IUT + Break MOQ scenario"
            : "No action scenario";
  const breakdownSubtitle =
    scenarioId === "iut"
      ...
      : scenarioId === "moq"
        ...
        : scenarioId === "iut-moq"
          ? `RM/PM stock, open POs, and leftover waste — IUT + MOQ scenario · ${row.cbuCode}`
          : scenarioId === "iut-moq-break"
            ? `RM/PM stock, open POs, and leftover waste — IUT + Break MOQ scenario · ${row.cbuCode}`
            : `RM/PM stock, open POs, and leftover waste — No action scenario · ${row.cbuCode}`;

  const transferScenarios: TransferScenarioId[] =
    scenarioId === "iut-moq" || scenarioId === "iut-moq-break"
      ? ["iut", "moq"]
  ...
  return (
    <div className="space-y-4 pt-2">
      ...
      {transferScenarios.length > 0 && (
        <div className="space-y-4">
          {transferScenarios.map((id) => (
            <StockTransferOptions key={id} scenarioId={id} />
          ))}
        </div>
      )}
