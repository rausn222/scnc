...
export function DetailDrawer({
  ...
  return createPortal(
    <div
      ...
      style={{ backgroundColor: BACKDROP_TINT, backdropFilter: "blur(2px)" }}
      onClick={onClose}
    >
      <motion.div
        initial={{ x: "100%" }}
        ...
        style={{ width, boxShadow: `-20px 0 60px ${BACKDROP_TINT}` }}
        onClick={(e) => e.stopPropagation()}
      >
        <div className="px-4 py-2.5 flex items-center justify-between gap-3 shrink-0" style={{ borderBottom: `1px solid ${C.border}` }}>
          <div className="min-w-0">
            <h2 className="text-xs font-bold" style={{ color: C.navy }}>{title}</h2>
            {subtitle && <p className="text-[10px] mt-0.5" style={{ color: C.muted }}>{subtitle}</p>}
          </div>
          <div className="flex items-center gap-2 shrink-0">
            ...
            <button
              ...
              style={{ color: C.muted }}
              onMouseEnter={(e) => { e.currentTarget.style.backgroundColor = C.bgSlate; }}
        ...
        {footer !== null && (
          <div className="px-4 py-3 flex justify-end shrink-0" style={{ borderTop: `1px solid ${C.border}` }}>
            {footer ?? (
              <button
                type="button"
                onClick={onClose}
