...
if (scenarioId === "custom-new") {
  return {
    ...
    extraIutRows: (editState.addedIutRows ?? []).map((row) => ({
      ...row,
      costPerTrip: autoIutCost(row.routeFrom, row.routeTo),
    })),
    ...
    procurementRows: (editState.addedProcurementRows ?? []).map((row) => {
      const automaticValues = autoProcurementValues(row.plant, row.matType, row.matCode, row.supplierName);
      return {
      id: row.id,
      plant: row.plant,
      matType: row.matType,
      ...
      moq: automaticValues.moq,
      pricePerUnit: automaticValues.pricePerUnit,
      total: row.orderQty * automaticValues.pricePerUnit,
      belowMoq: false,
      availableSuppliers: [],
      custom: true,
      };
    ...
    totalCost:
      (editState.addedIutRows ?? []).reduce(
        (sum, row) => sum + autoIutCost(row.routeFrom, row.routeTo),
        0,
      ) + (editState.addedProcurementRows ?? []).reduce(
        (sum, row) => sum + row.orderQty * autoProcurementValues(row.plant, row.matType, row.matCode, row.supplierName).pricePerUnit,
        0,
      ),
  };
}
  ...
  const addedProcurementRows: ScenarioProcurementRowVM[] = procurementApplicable
    ? editState.addedProcurementRows.map((row) => ({
        ...
        supplierName: row.supplierName,
        orderQty: row.orderQty,
        ...autoProcurementValues(row.plant, row.matType, row.matCode, row.supplierName),
        total: row.orderQty * autoProcurementValues(row.plant, row.matType, row.matCode, row.supplierName).pricePerUnit,
        belowMoq: false,
        availableSuppliers: [],
        custom: true,
      }))
  ...
  const extraIutCost = iutActive
    ? editState.addedIutRows.reduce((sum, row) => sum + autoIutCost(row.routeFrom, row.routeTo), 0)
    : 0;
