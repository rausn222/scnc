...
/** How long "Generate Scenario" spends in its loading state, simulating a real simulation run. */
const GENERATE_SCENARIOS_DELAY_MS = 900;
/** Shared loading-state delay for the drawer's commit actions (Save New Scenario / Save Changes,
    the Break-MOQ drawer's Done, and an Accept-reason submit) — same lightweight pattern as
    "Generate Scenario" and "Save Scenario", just under a distinct name for this group of actions. */
const CONFIRM_ACTION_DELAY_MS = 700;
// Waste-reduction-vs-baseline tiers used to color Business Waste savings (teal/amber/red) —
// reused across the ranked, custom-scenario, and custom-run row renderers below.
const SAVINGS_TIER_EXCELLENT_PCT = 40;
const SAVINGS_TIER_MODERATE_PCT = 20;
// Per-row simulated impact of a Custom Scenario override, used only by handleCustomScenarioRun.
const CUSTOM_WASTE_REDUCTION_PER_ROW = 110;
const CUSTOM_COVER_INCREASE_PER_ROW_DAYS = 0.5;
const ACCEPTED_BADGE_BORDER = "#86efac";
const CUSTOM_BADGE_BG = "#f5f3ff";
const CUSTOM_BADGE_TEXT = "#6d28d9";
const REASON_TEXTAREA_COLOR = "#1e293b";
const COMPARE_PENDING_BG = "#fff7ed";

function parseCurrencyValue(v: string | null | undefined): number {
  return v ? Math.max(0, parseFloat(v.replace(/[₹,]/g, "")) || 0) : 0;
}
...
export function ScenarioComparisonStep({
  ...
  const [isSubmittingAcceptReason, setIsSubmittingAcceptReason] = useState(false);
  ...
  // Loading state for the drawer footer's commit button (Save New Scenario / Save Changes) —
  // only meaningful while there are actual edits to persist; a plain "Done" close never sets it.
  const [isSavingDetailEdits, setIsSavingDetailEdits] = useState(false);
  ...
  const addCustomScenario = (businessWaste: string, fgDaysCover: string) => {
    ...
    const newScenario: ScenarioRow = {
      ...
      wasteColor: pct >= SAVINGS_TIER_EXCELLENT_PCT ? "teal" : "orange",
  ...
  const [isConfirmingBreakMoqDone, setIsConfirmingBreakMoqDone] = useState(false);
  const confirmBreakMoqDone = () => {
    if (isConfirmingBreakMoqDone) return;
    setIsConfirmingBreakMoqDone(true);
    window.setTimeout(() => {
      setIsConfirmingBreakMoqDone(false);
  ...
  // Comparison table pagination — the table's rows are ranked scenarios + user-created custom
  // scenarios + the No Action baseline, concatenated in that order (see render below). Reset to
  // page 1 whenever that combined row count changes (custom scenario added/removed, or the view
  // state toggles which row groups are shown).
  const [comparisonPage, setComparisonPage] = useState(1);
  const [comparisonRowsPerPage, setComparisonRowsPerPage] = useState(DEFAULT_ROWS_PER_PAGE);
  const comparisonTotalRows =
    (scenariosGenerated && scenarioViewState !== "editing" ? ranked.length : 0) +
    (scenarioViewState !== "editing" ? customScenarios.length : 0) +
    (scenariosGenerated && scenarioViewState === "default" ? 1 : 0);
  useEffect(() => {
    setComparisonPage(1);
  }, [comparisonTotalRows]);
  const comparisonTotalPages = Math.max(1, Math.ceil(comparisonTotalRows / comparisonRowsPerPage));
  ...
  const handleCustomScenarioRun = () => {
    ...
    ranked.forEach((s) => {
      ...
      recalculated[s.id] = {
        ...
        wasteColor: sPct >= SAVINGS_TIER_EXCELLENT_PCT ? "teal" : "orange",
      ...
      headerRight={
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            style={
              scenariosGenerated
                ? { backgroundColor: C.successBg, color: C.successText, border: `1px solid ${ACCEPTED_BADGE_BORDER}`, cursor: "default" }
                : isGeneratingScenarios
                  ? { backgroundColor: C.borderBlue, color: C.white, cursor: "default" }
                  : { backgroundColor: C.blue, color: C.white, cursor: "pointer" }
            }
          >
            {scenariosGenerated ? (
              <>
    ...
    >
      {isGeneratingScenarios ? (
        <div className="flex flex-col items-center justify-center gap-2 py-16">
          ...
          <p className="text-sm" style={{ color: C.muted }}>Generating scenario comparison…</p>
        </div>
        ...
        <>
          ...
          >
            ...
            <table className="text-xs w-full" style={{ borderCollapse: "collapse" }}>
              ...
              <tbody>
                {/* Ranked scenarios, custom scenarios, and the No Action baseline are paginated
                    together as one combined, ordered row list (10 per page by default). */}
                {(() => {
                  const rows: React.ReactNode[] = [];
                  /* Ranked scenarios — hidden while a custom scenario is being defined, shown (recalculated) once run */
                  if (scenariosGenerated && scenarioViewState !== "editing") {
                    rows.push(...ranked.map((scenario) => {
                    ...
                    >
                      ...
                      <td className="px-3 py-3">
                        <span
                          ...
                          style={{
                            color: displayWasteColor === "teal" ? C.teal : C.danger,
                          }}
                        >
                          {displayWaste}
                        </span>
                      ...
                      <td className="px-3 py-3">
                        ...
                        {coverDateLabel(displayCover) && (
                          <div className="tabular-nums" style={{ color: C.borderMuted, fontSize: 10 }}>
                      ...
                      <td className="px-3 py-3" style={{ borderLeft: `1px solid ${C.border}` }}>
                        <button
                          type="button"
                        ...
                        <button
                          ...
                          style={
                            inCompare
                              ...
                              : isMaxed
                                ? { backgroundColor: C.bgSlateLight, color: C.borderLight, border: `1px solid ${C.bgSlate}`, cursor: "not-allowed" }
                                : { backgroundColor: C.bgSlateLight, color: C.muted, border: `1px solid ${C.border}` }
                          }
                        >
                          {inCompare ? (
                        ...
                        {finalAcceptedId === scenario.id ? (
                          <div className="flex items-center gap-1.5">
                            <span
                              ...
                              style={{ backgroundColor: C.successBg, color: C.successText, fontSize: 10, border: `1px solid ${ACCEPTED_BADGE_BORDER}` }}
                            >
                              <Check size={11} />
                              Accepted
                            </span>
                            <button
                              ...
                              style={{ backgroundColor: C.blue, color: C.white, fontSize: 10 }}
                            >
                          ...
                          !finalAcceptedId && isSelected && (
                            <button
                              ...
                              className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg font-semibold whitespace-nowrap transition-all hover:opacity-90 cursor-pointer"
                              style={{ backgroundColor: C.green, color: C.white, fontSize: 10 }}
                            >
                              <Check size={11} />
                              Accept
                            </button>
                  ...
                  if (scenarioViewState !== "editing") {
                    rows.push(...customScenarios.map((scenario) => {
                  const isSelected = acceptedId === scenario.id;
                  const isAccepted = finalAcceptedId === scenario.id;
                  const highlighted = isSelected || isAccepted;
                  const { displayWaste, displaySavings, displayWasteColor, displayCover, coverDelta } =
                  ...
                  return (
                    <tr
                      ...
                      style={{
                        ...
                        backgroundColor: isAccepted ? C.successBg : highlighted ? C.bgBlue : C.white,
                        outline: isAccepted ? `2px solid ${C.green}` : highlighted ? `2px solid ${C.blue}` : undefined,
                        outlineOffset: highlighted ? -1 : undefined,
                        cursor: "pointer",
                      }}
                    >
                      <td className="px-3 py-3">
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          <span
                            className="px-1.5 py-0.5 rounded-full text-[9px] font-bold whitespace-nowrap"
                            style={{ backgroundColor: CUSTOM_BADGE_BG, color: CUSTOM_BADGE_TEXT }}
                          >
                            Custom
                          </span>
                          <button
                            ...
                            style={{ width: 18, height: 18, color: C.borderMuted }}
                          >
                            <Trash2 size={11} />
                          </button>
                        </div>
                      ...
                      <td className="px-3 py-3">
                        <span className="font-bold tabular-nums" style={{ color: displayWasteColor === "teal" ? C.teal : C.danger }}>
                          {displayWaste}
                        </span>
                        {displaySavings && (() => {
                          const saved = parseFloat((displaySavings ?? "").replace(/[₹,]/g, "")) || 0;
                  ...
                  if (scenariosGenerated && scenarioViewState === "default") {
                  ...
                  rows.push(
                    <tr
                      ...
                      style={{
                        borderBottom: `1px solid ${C.border}`,
                        borderLeft: isAccepted ? `3px solid ${C.green}` : highlighted ? `3px solid ${C.blue}` : "3px solid transparent",
                        backgroundColor: isAccepted ? BEST_TINT_BG : highlighted ? C.bgBlue : C.bgSlateLight,
                        outline: isAccepted ? `2px solid ${C.green}` : highlighted ? `2px solid ${C.blue}` : undefined,
                        outlineOffset: highlighted ? -1 : undefined,
                        cursor: "pointer",
                      }}
                    >
                      <td className="px-3 py-3">
                        <div className="flex items-center gap-2">
                          ...
                          <span
                            ...
                            style={{ backgroundColor: C.bgSlate, color: C.muted, border: `1px solid ${C.borderLight}` }}
                          >
                            BASE
                          </span>
                        </div>
                      ...
                      <td className="px-3 py-3">
                        <span className="font-bold tabular-nums" style={{ color: C.danger }}>
                  ...
                  return rows.slice(
                    (comparisonSafePage - 1) * comparisonRowsPerPage,
                    comparisonSafePage * comparisonRowsPerPage,
                  );
                })()}
                ...
                {scenarioViewState === "editing" && (
                  <tr>
                    <td colSpan={TABLE_HEADERS.length} className="px-3 py-8 text-center">
                      <span className="text-xs italic" style={{ color: C.borderMuted }}>
            ...
            {comparisonTotalRows > comparisonRowsPerPage && (
              <TablePagination
                page={comparisonSafePage}
                rowsPerPage={comparisonRowsPerPage}
                totalRows={comparisonTotalRows}
                onPageChange={setComparisonPage}
                onRowsPerPageChange={setComparisonRowsPerPage}
              />
            )}
          </div>
          ...
          <div className="mt-2 flex items-center justify-between flex-wrap gap-2">
            <div className="flex flex-col gap-1">
              ...
              <div className="flex items-center gap-3 flex-wrap">
                <span className="text-[10px]" style={{ color: C.borderMuted }}>Reduction %:</span>
                {([
                  { color: C.teal, label: "≥ 40%", meaning: "Excellent" },
                  { color: C.warning, label: "20–39%", meaning: "Moderate" },
                  { color: C.danger, label: "< 20%", meaning: "Poor" },
                ] as { color: string; label: string; meaning: string }[]).map(({ color, label, meaning }) => (
                  <span key={label} className="flex items-center gap-1">
                    <span className="inline-block w-2 h-2 rounded-full flex-shrink-0" style={{ backgroundColor: color }} />
                    <span className="text-[10px] font-semibold" style={{ color }}>{label}</span>
                    <span className="text-[10px]" style={{ color: C.borderMuted }}>{meaning}</span>
                  </span>
                ))}
              </div>
            </div>
            <div className="flex items-center gap-2">
              {compareIds.size > 0 && (
                <span
                  ...
                  style={{
                    backgroundColor: compareIds.size < 2 ? COMPARE_PENDING_BG : compareIds.size >= 3 ? C.successBg : C.bgBlue,
                    color: compareIds.size < 2 ? ATTENTION_TEXT : compareIds.size >= 3 ? C.successText : C.blue,
                  }}
                >
                  {compareIds.size < 2
          ...
          {detailScenario && detailFields && (
              ...
              headerRight={
                showsEditableDetailTable && !isCustomising ? (
                  <button
                    ...
                    style={{ backgroundColor: C.white, color: C.navy, border: `1px solid ${C.borderLight}` }}
                  >
                    <Pencil size={11} />
                    Customise
                  </button>
                  ...
                  return (
                    <div className="w-full flex items-center justify-between gap-3">
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        ...
                        <button
                          type="button"
                          disabled={blockedByLimit || isSavingDetailEdits}
                          ...
                          style={
                            blockedByLimit
                              ? { backgroundColor: C.bgSlate, color: C.borderLight, cursor: "not-allowed" }
                              : { backgroundColor: C.blue, color: C.white, cursor: isSavingDetailEdits ? "default" : "pointer" }
                          }
                        >
                          {isSavingDetailEdits ? (
            ...
            >
              {isCustomDetailScenario && detailScenarioSnapshot ? (
                ...
                <div className="p-6">
                  <div className="rounded-2xl p-5 flex flex-col gap-3" style={{ border: `1px solid ${C.border}`, backgroundColor: NEUTRAL_CARD_BG }}>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="px-1.5 py-0.5 rounded-full text-[9px] font-bold" style={{ backgroundColor: CUSTOM_BADGE_BG, color: CUSTOM_BADGE_TEXT }}>
                        Custom
                      </span>
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      <div className="rounded-lg px-3 py-2.5 text-center" style={{ backgroundColor: C.white, border: `1px solid ${C.border}` }}>
          ...
          {showAcceptReasonModal && createPortal(
            ...
            >
              ...
              >
                <div className="px-5 py-4" style={{ borderBottom: `1px solid ${C.border}`, backgroundColor: C.warningBgLight }}>
                  <div className="flex items-start gap-3">
                    <div className="mt-0.5 flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center" style={{ backgroundColor: C.warningBg }}>
                      <Star size={15} style={{ color: C.warning }} />
                    </div>
                    <div>
                      <p className="text-sm font-bold" style={{ color: C.warningTextDark }}>Better option available</p>
                      <p className="text-xs mt-0.5" style={{ color: C.warningText }}>
                ...
                <div className="px-5 py-4 flex flex-col gap-3">
                  <textarea
                    ...
                    style={{ border: `1px solid ${C.borderLight}`, color: REASON_TEXTAREA_COLOR, lineHeight: 1.6 }}
                    autoFocus
                  />
                  <div className="flex items-center justify-end gap-2">
                    ...
                    <button
                      type="button"
                      disabled={!acceptReasonText.trim() || isSubmittingAcceptReason}
                      onClick={() => {
                        if (isSubmittingAcceptReason) return;
                        setIsSubmittingAcceptReason(true);
                        window.setTimeout(() => {
                          setIsSubmittingAcceptReason(false);
                      ...
                      className="px-4 py-1.5 rounded-lg text-xs font-semibold transition-opacity inline-flex items-center gap-1.5"
                      style={{
                        backgroundColor: acceptReasonText.trim() ? C.blue : C.borderBlue,
                        color: C.white,
                        cursor: acceptReasonText.trim() && !isSubmittingAcceptReason ? "pointer" : "not-allowed",
                      }}
                    >
                      {isSubmittingAcceptReason && <Loader2 size={12} className="animate-spin" />}
                      {isSubmittingAcceptReason ? "Submitting…" : "Submit & Accept"}
                    </button>
                  </div>
                </div>
              </div>
              ...
              >
                ...
                <div className="px-5 py-4 flex flex-col gap-3">
                  ...
                  <div className="flex items-center justify-end gap-2">
                    ...
                    <button
                      type="button"
                      disabled={!breakMoqDoneReasonText.trim() || isConfirmingBreakMoqDone}
                      ...
                      className="px-4 py-1.5 rounded-lg text-xs font-semibold transition-opacity inline-flex items-center gap-1.5"
                      style={{
                        backgroundColor: breakMoqDoneReasonText.trim() ? C.blue : C.borderBlue,
                        color: C.white,
                        cursor: breakMoqDoneReasonText.trim() && !isConfirmingBreakMoqDone ? "pointer" : "not-allowed",
                      }}
                    >
                      {isConfirmingBreakMoqDone && <Loader2 size={12} className="animate-spin" />}
                      {isConfirmingBreakMoqDone ? "Confirming…" : "Done"}
                    </button>
                  </div>
                </div>
              </div>
