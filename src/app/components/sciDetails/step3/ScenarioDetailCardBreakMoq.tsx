...
import { ArrowLeftRight, Check, Clock, Link2Off, ShoppingCart, Star } from "lucide-react";
import type { IUTOption, MOQPlantOption, PlantRole } from "../types";
import { C, IUT_TRANSFER_OPTIONS, MOQ_PLANT_OPTIONS_BREAK, PLANT_BREAKDOWN_BASE, PM_BADGE, RM_BADGE } from "../constants";
...
function StatTile({ label, value, tone }: { label: string; value: string; tone: "cost" | "fg" }) {
  const palette = tone === "cost" ? { label: "#c2410c", value: "#c2410c" } : { label: "#166534", value: "#166534" };
  return (
        ...
        return (
          ...
          >
            ...
            <div className="p-3.5 flex flex-col gap-3">
              ...
              <div className="rounded-lg px-3 py-2.5 flex flex-col gap-2" style={{ border: `1px solid ${isCustomising ? C.blue : "#e2e8f0"}` }}>
                <div className="flex items-center gap-2.5 flex-wrap">
                  <ArrowLeftRight size={13} style={{ color: C.blue }} className="shrink-0" />
              ...
              {/* Plant impact — compact one-liner per plant */}
              <div className="flex flex-col gap-1">
                {plantOutcomes.map((o) => (
                  <div key={o.plantCode} className="flex items-center justify-between gap-2 rounded-lg px-2.5 py-1.5 flex-wrap" style={{ backgroundColor: "#fff", border: "1px solid #f1f5f9" }}>
                    <span className="text-[11px] font-bold shrink-0" style={{ color: C.navy }}>{o.plantCode}</span>
                    <span className="text-[11px] tabular-nums" style={{ color: "#374151" }}>
                      {formatIndianNumber(o.finalFgProducible)} EA · Stop {o.productionStopDate}
                    </span>
