...
import { ScenarioDetailCard } from "./ScenarioDetailCardTabular";
...
export function ScenarioComparisonStep({
  ...
  const viewDetails = (scenario: ScenarioRow) => {
    ...
    navigate({
      ...
      srNo: row.srNo,
      from: { page: "sci-detail", srNo: row.srNo },
    });
  };
      ...
      headerRight={
        <button
          ...
          onClick={() => {
            ...
            // Clear any previously selected scenario's detail card — it shouldn't
            // linger while defining/running a custom scenario.
            if (acceptedId) onSelect(acceptedId);
          }}
