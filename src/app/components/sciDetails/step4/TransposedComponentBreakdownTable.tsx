...
import type { ComponentBreakdownRow, OnHandBreakdown, PlantRole } from "../types";
import {
  C,
...
import { parseIndianNumber, formatIndianNumber, parseDMYDate } from "../utils";
...
const PLANT_COL_WIDTH = 120;
const MATERIAL_COL_WIDTH = 150;
const METRIC_COL_WIDTH = 140;
...
export function TransposedComponentBreakdownTable({
  ...
  const onHandColSpan = stockExpanded ? 1 + ON_HAND_BREAKDOWN_ROWS.length : 1;
  ...
  const metricsWidth = TRANSPOSED_METRICS.reduce(
    (sum, m) => sum + (m.id === "onHand" ? onHandColSpan * METRIC_COL_WIDTH : METRIC_COL_WIDTH),
    0,
  );
      ...
      <div style={{ flexShrink: 0 }}>
        ...
        >
          ...
          <thead style={{ position: "sticky", top: 0, zIndex: 4 }}>
            ...
            {/* Empty row matching the metrics pane's on-hand sub-header row, so the two
                panes' header heights stay equal and body rows stay aligned across them. */}
            {stockExpanded && (
              <tr style={{ backgroundColor: EXPANDED_BREAKDOWN_CELL_BG }}>
                <th className="px-1.5 py-1" style={subHeaderStyle} />
                <th className="px-1.5 py-1" style={subHeaderStyle} />
              </tr>
            )}
          </thead>
                    ...
                    >
                      <div className="flex flex-col items-center text-center gap-1">
                        ...
                        <span
                          ...
                          style={{ color: C.navy }}
                        >
                          {formatPlantRoleLabel(group.roles)}
                        </span>
                        <button
                          type="button"
      ...
      <div className="cbp-scroll" style={{ overflowX: "auto", overflowY: "hidden", flex: "1 1 0%", minWidth: 0 }}>
        ...
        >
                ...
                <Fragment key="onhand-cols">
                  ...
                  {stockExpanded &&
                    ON_HAND_BREAKDOWN_ROWS.map((sub) => (
                      <col key={sub.key} style={{ width: SUB_COL_WIDTH }} />
                    ))}
                </Fragment>
              ) : (
          ...
          <thead style={{ position: "sticky", top: 0, zIndex: 4 }}>
              ...
              {TRANSPOSED_METRICS.map((metric) => {
                if (metric.id === "onHand") {
                  return (
                    <th
                      key={metric.id}
                      colSpan={onHandColSpan}
                      rowSpan={stockExpanded ? 1 : 2}
                ...
                return (
                  <th
                    ...
                    rowSpan={2}
                    className="px-1.5 py-1 uppercase"
                    style={metricHeaderStyle("highlight" in metric && metric.highlight === true)}
                  >
                    {metric.label}
                  </th>
                );
            ...
            {stockExpanded && (
              <tr style={{ backgroundColor: EXPANDED_BREAKDOWN_CELL_BG }}>
                <th className="px-1.5 py-1 uppercase" style={subHeaderStyle}>
                  On-hand
                </th>
                {ON_HAND_BREAKDOWN_ROWS.map((sub) => (
                  <th key={sub.key} className="px-1.5 py-1 uppercase" style={subHeaderStyle}>
                    {sub.label}
                  </th>
                ))}
              </tr>
                        ...
                        <Fragment key="onHand">
                          <td
                            ...
                            style={{
                              ...
                              borderRight: stockExpanded ? "1px solid #e2e8f0" : dataBorderRight,
                              borderBottom: "1px solid #e2e8f0",
                              height: ROW_HEIGHT,
                            }}
                          >
                            <div className="flex justify-center items-center h-full">
                              <TransposedMetricCell metricId="onHand" before={col.before} after={col.after} />
                            </div>
                          </td>
                          {stockExpanded &&
                            ON_HAND_BREAKDOWN_ROWS.map((sub) => (
