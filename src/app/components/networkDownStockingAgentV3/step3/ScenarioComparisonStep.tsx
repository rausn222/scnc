...
export function ScenarioComparisonStep({
  ...
  const viewDetails = (scenario: ScenarioRow) => {
    ...
    navigate({
      ...
      from: { page: "network-down-stocking-agent-v3", srNo: row.srNo },
    });
  };
  ...
  // "IUT + Procurement" row's More Details — its own dedicated drawer (IutProcurementDrawerV1)
  // holding a single wide, sticky-left-column marksheet table, tracked separately from the
  // generic DetailDrawer below since its content isn't a per-scenario breakdown.
  const [showIutDrawer, setShowIutDrawer] = useState(false);
  ...
  const closeDetailDrawer = () => {
    ...
    setDetailViewMode("card");
  };

  // Done on the "IUT + Break MOQ" drawer: closes normally if the recommended option is selected,
  // otherwise requires a reason first before the drawer is allowed to close.
  ...
  return (
    ...
    >
            ...
            {scenarioViewState !== "editing" && ranked.map((scenario) => {
              ...
              return (
                ...
                >
                  ...
                  {/* More Details — every row opens a right-side drawer. IUT + Procurement gets
                      its own dedicated marksheet drawer (same style as V2's "IUT + Procurement
                      - v1"); every other row opens the generic DetailDrawer with its existing
                      breakdown content (Card/Table toggle, MOQ-break view, etc.) unchanged. */}
                  <td className="px-3 py-3" style={{ borderLeft: "1px solid #e2e8f0" }}>
                    <button
                      type="button"
                      onClick={(e) => {
                        if (scenario.id === "iut-moq") {
                          e.stopPropagation();
                          onSelect(scenario.id);
                          setShowIutDrawer(true);
                        } else {
                          openDetail(scenario, e);
                        }
                      }}
            ...
            <div className="p-4 space-y-4">
              {detailScenario.id === "iut-moq-break" ? (
                <ScenarioDetailCardBreakMoq
                  ...
                  maxCustomScenarios={MAX_CUSTOM_SCENARIOS}
                  onAddCustomScenario={addCustomScenario}
                  onCloseDrawer={closeDetailDrawer}
                />
              ) : (
                <>
      ...
      {/* "IUT + Procurement" row's More Details — its own dedicated drawer holding just the
          sticky-left-column marksheet comparison table (same style as V2's
          "IUT + Procurement - v1"), separate from the generic DetailDrawer above. */}
      {showIutDrawer && (() => {
        const iutBase = ranked.find((s) => s.id === "iut-moq");
        if (!iutBase) return null;
        const iutFields = displayFieldsFor(iutBase);
        return (
          <IutProcurementDrawerV1
            title="IUT + Procurement"
            subtitle={`${iutFields.displayWaste} business waste · ${iutFields.displayCover} FG cover`}
            onClose={() => setShowIutDrawer(false)}
          />
        );
      })()}
