...
type MaterialScopeGroup = {
  ...
  materialCode: string;
  description: string;
  contributions: Array<{ oldCbu: string; contributionType: string }>;
};

const MATERIAL_SCOPE_GROUPS: MaterialScopeGroup[] = [
  { materialType: "PM", materialCode: "11477867", description: "VAS ALOE FRESH 100ML FENOMENO CAP", contributions: [
    { oldCbu: "VAFA1R3", contributionType: "Unique" },
    { oldCbu: "VAFA1R0", contributionType: "High Contribution" },
    { oldCbu: "VCBL1R0", contributionType: "High contribution" },
  ] },
  { materialType: "PM", materialCode: "65284824", description: "85ml Bottle Cap & Shrink Sleeve PM", contributions: [
    { oldCbu: "VAFA1R3", contributionType: "Unique" },
  ] },
  { materialType: "RM", materialCode: "11100345", description: "VAS ALOE FRESH 50ML", contributions: [
    { oldCbu: "VCBL1R0", contributionType: "Unique" },
    { oldCbu: "VAFA1R3", contributionType: "Unique" },
  ] },
];

function MaterialScopeContentOld({ selected, onToggle }: { selected: Record<string, boolean>; onToggle: (materialCode: string) => void }) {
  return (
    <div className="p-5">
      <div className="overflow-x-auto rounded-xl border border-slate-200">
        <table className="w-full min-w-[820px] border-collapse text-sm">
          <thead className="bg-[#0b3b91] text-white"><tr>
            {["Tick", "Material Type", "Material Code", "Description", "Old CBU List", "Contribution Type"].map((heading) => (
              <th key={heading} className="px-4 py-3 text-left text-[10px] text-xs font-semibold uppercase tracking-wide">{heading}</th>
            ))}
          </tr></thead>
          <tbody>
            {MATERIAL_SCOPE_GROUPS.flatMap((material) => material.contributions.map((contribution, index) => (
              <tr key={`${material.materialCode}-${contribution.oldCbu}`} className="border-t border-slate-200 hover:bg-slate-50">
                {index === 0 && <>
                  <td rowSpan={material.contributions.length} className="px-4 py-3 align-top">
                    <input type="checkbox" checked={selected[material.materialCode] ?? false} onChange={() => onToggle(material.materialCode)} aria-label={`Select material ${material.materialCode}`} className="h-4 w-4 accent-[#1769c2]" />
                  </td>
                  <td rowSpan={material.contributions.length} className="px-4 py-3 text-[10px] align-top font-medium text-slate-700">{material.materialType}</td>
                  <td rowSpan={material.contributions.length} className="px-4 py-3 text-[10px] align-top font-semibold text-[#1454a3]">{material.materialCode}</td>
                  <td rowSpan={material.contributions.length} className="px-4 py-3 text-[10px] align-top text-slate-700">{material.description}</td>
                </>}
                <td className="px-4 py-3 text-[10px] text-slate-700">{contribution.oldCbu}</td>
                <td className="px-4 py-3 text-[10px] text-slate-700">{contribution.contributionType}</td>
              </tr>
            )))}
          </tbody>
        </table>
